...
Furniture that still needs to read as a link uses the quiet pattern: `color: inherit` with an underline in `--hairline-strong`, going `--accent` + `currentColor` on hover (`ToolList.astro › .row__link`, `ReadingList.astro › .row__domain`, `tools/[slug].astro › .strip a`, which the repo link at the foot joins rather than restating). It carries its own underline because on touch there is no hover to reveal one.
...
2. `npm test` → all pass (235 at the time of writing).
