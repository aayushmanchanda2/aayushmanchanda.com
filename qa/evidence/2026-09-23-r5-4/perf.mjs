// R5-4 scroll probe (from VET-262 perf.mjs): frame times over 2s of scrolling plus mat/stamp invalidations.
// repaint during the scroll (a repainting mat would show as a full-window
// 1280x800 paint on each frame).
// usage: node perf.mjs [route] [base]
import { chromium } from "playwright";

const [route = "/library", base = "http://localhost:4337"] = process.argv.slice(2);
...
const node = (re) => inval.filter((e) => re.test(JSON.stringify(e.args).match(/nodeName":"([^"]*)/)?.[1] ?? "")).length;
const matInval = node(/class='mat/);
const stampInval = node(/stamp|postmark/);
const perf = { stampInvalidations: stampInval, paintMs: ms("Paint").toFixed(1), rasterMs: ms("RasterTask").toFixed(1), invalidations: inval.length, matInvalidations: matInval };
