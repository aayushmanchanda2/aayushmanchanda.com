...
const BEFORE = "qa/evidence/2026-09-23-vet-274/dist-before";
const changed = files("dist").filter((f) => {
  const old = path.join(BEFORE, path.relative("dist", f));
  return !existsSync(old) || !readFileSync(old).equals(readFileSync(f));
});
const untracked = execFileSync("git", ["ls-files", "--others", "--exclude-standard", "--", ".", ":!qa/evidence"], { encoding: "utf8" }).split("\n").filter(Boolean);
const corpora = {
  distChanged: changed.map((f) => readFileSync(f, "latin1")).join("\n"),
...
const result = { archiveRows: archive.length, alsoPublicOnMain: archive.length - hidden.length, terms: terms.length, distChangedFiles: changed.length, untrackedFiles: untracked.length };
