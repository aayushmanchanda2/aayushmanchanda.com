// VET-260 scroll probe: frame times while /tools scrolls, and every layer
// repaint during the scroll (a repainting mat would show as a full-window
// 1280x800 paint on each frame).
// usage: node perf.mjs [variant] [route]
import { chromium } from "playwright";

const [variant = "a", route = "/tools"] = process.argv.slice(2);
...
console.log(JSON.stringify({ variant, route, ...frames, ...perf }));
await b.close();
